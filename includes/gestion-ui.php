...
function montseny_render_gestion() {
    ...
    // --- PROCESAR ACCIONES ANTES DE PINTAR ---
    ...
    if (isset($_POST['m_import']) && !empty($_FILES['csv']['tmp_name'])) {
        ...
        while (($data = fgetcsv($handle, 0, $sep)) !== FALSE) {
            ...
            if ( !is_wp_error($uid) ) {
                ...
                $wpdb->insert($tabla, array('user_id' => $uid, 'ramo' => $data[5], 'observaciones' => $data[10], 'fecha_alta' => date('Y-m-d', strtotime($data[8]))));
                $ex++;
            } else { $er++; }
        }
    ...
    if (isset($_POST['m_alta_manual'])) {
        ...
        if ( !is_wp_error($uid) ) {
            ...
            $u = new WP_User($uid); $u->set_role('afiliade');
            $wpdb->insert($tabla, array('user_id' => $uid, 'ramo' => sanitize_text_field($_POST['ra']), 'observaciones' => sanitize_textarea_field($_POST['ob']), 'fecha_alta' => date('Y-m-d')));
    ...
    if (isset($_POST['save_edit'])) {
        ...
        $wpdb->update($tabla, array('ramo'=>$_POST['ra'], 'etiquetas'=>$_POST['ta'], 'observaciones'=>$_POST['ob'], 'direccion_cifrada'=>Montseny_Crypto::encrypt($_POST['di']), 'iban_cifrado'=>Montseny_Crypto::encrypt($_POST['ib'])), array('user_id'=>$uid));
    ...
    <style>
        ...
        .btn { background: #CC0000; color: #fff; padding: 12px; border-radius: 6px; text-decoration: none; border:none; font-weight:bold; cursor:pointer; display:inline-block; }
        .card-box { background: #222; padding: 15px; border-radius: 8px; margin-bottom: 20px; }
        input, textarea { width: 100%; padding: 12px; margin: 10px 0; background: #333; color: #fff; border: 1px solid #444; border-radius: 5px; box-sizing: border-box; }
    ...
    <div class="container">
        ...
        <?php if(isset($_GET['msg'])): ?>
            <div class="msg msg-ok">✅ Acción realizada correctamente.</div>
        <?php endif; ?>

        <div style="margin-bottom:20px; display:flex; gap:10px;">
            <a href="?view=alta" class="btn">➕ AÑADIR</a>
            <a href="?view=import" class="btn" style="background:#444;">📥 IMPORTAR</a>
        </div>
        ...
        <?php if (isset($_GET['view']) && $_GET['view'] == 'alta') : ?>
            <div class="card-box">
                <form method="post">
                    ...
                    <button type="submit" name="m_alta_manual" class="btn" style="width:100%">CREAR</button>
        ...
        ?>
            <div class="card-box">
                <form method="post">
                    ...
                    <button type="submit" name="save_edit" class="btn" style="width:100%">GUARDAR</button>
        ...
        <br><a href="<?php echo site_url('/montseny'); ?>" style="color:#666; font-size:0.8rem;">← Volver a la App</a>
    </div></body></html>
    <?php
}
